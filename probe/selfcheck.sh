...
python3 - <<'DIFPY' || FAILED=1
import io, json, re, subprocess, sys

# differed.json says, per participant and case, why the answer differs: a limit of that type system,
# a type never resolved, or a divergence. It is written by hand, so what is checked here is that it
# describes this run and no other - every differing cell classified, and nothing classified that no
# longer differs. Without that it would drift into describing a measurement that has moved.
COLS = [("PYTHON", "py"), ("GO", "go"), ("VALIDATOR", "py"), ("ISTHMUS", "calcite"),
        ("DATAFUSION", "df"), ("DUCKDB", "duckdb"), ("SPARK", "spark"), ("ACERO", "acero")]
doc = json.load(open("differed.json", encoding="utf-8"))
bad = 0
for rid, r in doc["rules"].items():
    if r["kind"] not in doc["kinds"]:
        print("FAILED: rule %s has an unknown kind %r" % (rid, r["kind"])); bad = 1
...
for col, fmt in COLS:
    ...
    if missing:
        print("FAILED: %s: %d differing cells with no reason: %s"
              % (col, len(missing), ", ".join(missing[:4]))); bad = 1
    if extra:
        print("FAILED: %s: %d classified cells that no longer differ: %s"
              % (col, len(extra), ", ".join(extra[:4]))); bad = 1
    for case, rid in said.items():
        if rid not in doc["rules"]:
            print("FAILED: %s/%s names rule %r, which is not defined" % (col, case, rid)); bad = 1
    total += len(differing)

kinds = {}
for col, cs in doc["cells"].items():
    for rid in cs.values():
        k = doc["rules"].get(rid, {}).get("kind")
...
for sentence in ("gives all %d of them a reason and marks %d as something other than"
                 ...
                 "%s of its %s reasons name an issue"
                 % (WORD.get(filed), WORD.get(len(doc["rules"])))):
    if sentence not in " ".join(readme.split()):
        print("FAILED: the README does not say %r" % sentence); bad = 1

if not bad:
    print("ok      %d differing cells, %d reasons (%d naming an issue), %s"

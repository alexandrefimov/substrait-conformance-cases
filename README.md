...
column of wrong answers, and a limit of a type system is hatched rather than red. The picture is
generated by `probe/heatmap.py` from the same columns as the table below;
[the same matrix as a page](https://alexandrefimov.github.io/substrait-conformance-cases/) puts the
expectation and the answer under the cursor.
...
| `docs/` | the matrix drawn: `matrix.svg` and `matrix-dark.svg` for the README, `index.html` for the site, all three written by `probe/heatmap.py` |
